...
function startFunPhrases() {
  const phrases = tm('login.fun_phrases')
  phraseInterval = setInterval(() => {
    textOpacity.value = 0
    setTimeout(() => {
      loadingText.value = phrases[Math.floor(Math.random() * phrases.length)]
      textOpacity.value = 1
    }, 500)
  }, 3000)
}
    ...
    <div v-else class="max-w-sm w-full flex flex-col items-center">
      ...
      <div class="w-full bg-card border border-border/50 rounded-[32px] p-8 shadow-2xl relative mb-8">
        <div class="text-center mb-6">
          <h2 class="text-lg font-bold">{{ $t('login.sign_in_telegram') }}</h2>
        </div>
