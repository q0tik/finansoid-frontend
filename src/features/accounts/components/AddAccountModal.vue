      ...
      <div class="bg-card text-card-foreground rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-border animate-in zoom-in-95 duration-200 relative">
        ...
        <div class="flex justify-between items-center mb-6">
          <div class="flex items-center gap-2">
            ...
            <h2 class="text-xl font-bold tracking-tight">{{ $t('account.new_account') }}</h2>
          </div>
          <button 
            @click="emits('close')" 
        ...
        <form @submit.prevent="handleSubmit" class="space-y-5">
          <div class="space-y-2">
            <label class="text-sm font-medium ml-1">{{ $t('account.title') }}</label>
            <input 
              id="acc-title" 
              v-model="title" 
              type="text" 
              ...
              :placeholder="$t('account.title_placeholder')"
              required 
            />
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
              <label class="text-sm font-medium ml-1">{{ $t('account.balance') }}</label>
              <input 
                v-model="balance" 
                type="number" 
                step="0.01" 
            ...
            <div class="space-y-2 relative">
              <label class="text-sm font-medium ml-1">{{ $t('account.currency') }}</label>
