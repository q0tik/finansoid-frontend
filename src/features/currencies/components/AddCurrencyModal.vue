      ...
      >
        <div class="flex justify-between items-center mb-6">
          <div class="flex items-center gap-2">
            ...
            <h2 class="text-xl font-bold tracking-tight">{{ $t('currency.add') }}</h2>
          </div>
          <button
            @click="emits('close')"
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div class="space-y-2">
            <label for="currency-title" class="text-sm font-medium ml-1">{{ $t('currency.title') }}</label>
            <input
              id="currency-title"
              v-model="title"
              type="text"
              ...
              :placeholder="$t('currency.title_placeholder')"
              required
            />
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
              <label for="code" class="text-sm font-medium ml-1">{{ $t('currency.code') }}</label>
              <input
                id="code"
                v-model="code"
                type="text"
            ...
            <div class="space-y-2">
              <label for="symbol" class="text-sm font-medium ml-1">{{ $t('currency.symbol') }}</label>
              <input
                id="symbol"
                v-model="symbol"
                type="text"
          ...
          <div class="space-y-2">
            <label for="decimals" class="text-sm font-medium ml-1">{{ $t('currency.decimal_digits') }}</label>
            <input
              id="decimals"
              v-model="decimals"
              type="number"
