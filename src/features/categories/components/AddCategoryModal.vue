      ...
      <div class="bg-card text-card-foreground rounded-2xl shadow-xl w-full max-w-sm p-6 border border-border animate-in zoom-in-95 duration-200">
        ...
        <div class="flex justify-between items-center mb-6">
          <div class="flex items-center gap-2">
            ...
            <h2 class="text-xl font-bold tracking-tight">{{ $t('category.add_title') }}</h2>
          </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-5">
          <div class="space-y-2">
            <label for="category-title" class="text-sm font-medium ml-1">{{ $t('category.title_label') }}</label>
            <input
              id="category-title"
              v-model="title"
              type="text"
          ...
          <div class="space-y-2">
            <label class="text-sm font-medium ml-1">{{ $t('category.type') }}</label>
            <div class="grid grid-cols-2 gap-2 p-1 bg-muted rounded-xl">
              ...
              >
                <TrendingDown class="w-4 h-4" /> {{ $t('category.expense') }}
              </button>
              <button
                type="button"
                @click="isIncome = true"
              ...
              >
                <TrendingUp class="w-4 h-4" /> {{ $t('category.income') }}
              </button>
            </div>
          </div>
