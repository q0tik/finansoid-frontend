...
<template>
  <div v-if="loading" class="flex flex-col items-center justify-center py-20 text-muted-foreground">
    ...
    <span class="text-sm text-emerald-500/50">{{ $t('transaction.loading_categories') }}</span>
  </div>
  ...
  <form v-else @submit.prevent="handleSubmit" class="space-y-6 pt-2 pb-20">
    ...
    <div class="space-y-3">
      <div class="relative">
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <span v-else class="text-muted-foreground">{{ $t('transaction.source_of_income') }}</span>
          </div>
        ...
        <transition name="dropdown">
          <div v-if="activeDropdown === 'category'" class="absolute z-50 w-full mt-2 bg-card border border-border shadow-2xl rounded-2xl overflow-hidden">
            <div class="max-h-[200px] overflow-y-auto p-1 custom-scrollbar">
              ...
              >
                <span :class="{ 'font-bold text-emerald-600': categoryId === c.id }">{{ categoryTitle(c) }}</span>
      ...
      <div class="relative">
        <label class="text-[11px] font-bold uppercase tracking-wider text-muted-foreground ml-4 mb-1 block opacity-60">{{ $t('transaction.to_account') }}</label>
        <div 
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <span v-else class="text-muted-foreground">{{ $t('transaction.select_target') }}</span>
          </div>
      ...
      <div class="space-y-1">
        <label class="text-[11px] font-bold uppercase tracking-wider text-muted-foreground ml-4 block opacity-60">{{ $t('transaction.comment') }}</label>
        <div class="relative">
          ...
          <textarea
            v-model="comment"
            rows="1"
            :placeholder="$t('transaction.income_comment_placeholder')"
    ...
    <Toast 
      :show="showToast" 
      :message="$t('transaction.income_saved')"
