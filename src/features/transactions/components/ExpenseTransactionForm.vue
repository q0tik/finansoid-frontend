...
async function handleSubmit() {
  ...
  try {
    ...
    alert(t('transaction.error_creating'))
  }
}
...
<template>
  <div v-if="loading" class="flex flex-col items-center justify-center py-20 text-muted-foreground">
    ...
    <span class="text-sm">{{ $t('transaction.loading_data') }}</span>
  </div>
  ...
  <form v-else @submit.prevent="handleSubmit" class="space-y-6 pt-2 pb-20">
    ...
    <div class="space-y-3">
      <div class="relative">
        <label class="text-[11px] font-bold uppercase tracking-wider text-muted-foreground ml-4 mb-1 block opacity-60">{{ $t('transaction.category') }}</label>
        <div 
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <span v-if="selectedCategory" class="font-bold">{{ categoryTitle(selectedCategory) }}</span>
            <span v-else class="text-muted-foreground">{{ $t('transaction.choose_category') }}</span>
          </div>
        ...
        <transition name="dropdown">
          <div v-if="activeDropdown === 'category'" class="absolute z-50 w-full mt-2 bg-card border border-border shadow-2xl rounded-2xl overflow-hidden">
            <div class="max-h-[200px] overflow-y-auto p-1 custom-scrollbar">
              ...
              >
                <span :class="{ 'font-bold text-primary': categoryId === c.id }">{{ categoryTitle(c) }}</span>
      ...
      <div class="relative">
        <label class="text-[11px] font-bold uppercase tracking-wider text-muted-foreground ml-4 mb-1 block opacity-60">{{ $t('transaction.account') }}</label>
        <div 
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <span v-else class="text-muted-foreground">{{ $t('transaction.select_source') }}</span>
          </div>
      ...
      <div class="space-y-1">
        <label class="text-[11px] font-bold uppercase tracking-wider text-muted-foreground ml-4 block opacity-60">{{ $t('transaction.comment_optional') }}</label>
        <div class="relative">
          ...
          <textarea
            v-model="comment"
            rows="1"
            :placeholder="$t('transaction.expense_comment_placeholder')"
    ...
    >
      {{ $t('transaction.create_expense') }}
    </Button>

    <Toast 
      :show="showToast" 
      :message="$t('transaction.expense_saved')"
