...
async function handleSubmit() {
  ...
  if (needsExchange.value && !receivedAmount.value) {
    alert(t('transaction.specify_exchange'))
    return
  }
  try {
    ...
    alert(t('transaction.error_transfer'))
  }
}
...
<template>
  <div v-if="loading" class="flex flex-col items-center justify-center py-20 text-muted-foreground">
    ...
    <span class="text-sm">{{ $t('transaction.loading_accounts') }}</span>
  </div>
  ...
  <form v-else @submit.prevent="handleSubmit" class="space-y-6 pt-2 pb-20">
    ...
    <div class="space-y-1 relative">
      ...
      <div class="relative">
        ...
        >
          <div class="flex items-center gap-3">
            <div class="p-2 bg-blue-500/10 text-blue-500 rounded-xl font-bold text-xs uppercase">{{ $t('transaction.from') }}</div>
      ...
      <div class="relative">
        ...
        >
          <div class="flex items-center gap-3">
            <div class="p-2 bg-purple-500/10 text-purple-500 rounded-xl font-bold text-xs uppercase">{{ $t('transaction.to') }}</div>
    ...
    <transition name="slide-fade">
      <div v-if="needsExchange" class="bg-muted/30 border border-border/50 rounded-2xl p-4 space-y-4">
        <div class="flex items-center gap-2 text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">
          ...
          <span>{{ $t('transaction.currency_exchange') }}</span>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-muted-foreground ml-1">
              {{ $t('transaction.rate', { symbol: fromAccount?.currency_symbol }) }}
            </label>
            <input 
          ...
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-muted-foreground ml-1">
              {{ $t('transaction.received', { symbol: toAccount?.currency_symbol }) }}
            </label>
            <div class="relative">
              <input 
    ...
    <div class="space-y-1">
      <div class="relative">
        ...
        <textarea
          v-model="comment"
          rows="1"
          :placeholder="$t('transaction.transfer_comment_placeholder')"
    ...
    >
      {{ $t('transaction.transfer_money') }}
    </Button>

    <Toast 
      :show="showToast" 
      :message="$t('transaction.transfer_saved')"
